...
const TemporalComponent = () => {
  ...
  const fetchData = () => {
    getRecs(NUM_CLICKS).then((data) => setData(data));
  };
  ...
  const renderLinks = (read: boolean) => {
    ...
    const clicks = getClicksSummary(NUM_CLICKS);
  ...
  return (
    <div className="App">
      <Page className="App-header">
        <h3>Top 5 articles (based on last {NUM_CLICKS} clicks)</h3>
        {renderLinks(false)}
        <h3>Read articles (last {NUM_CLICKS} only)</h3>
        {renderLinks(true)}
      </Page>
    </div>
  );
...
export const Temporal = withBasePage(TemporalComponent);
